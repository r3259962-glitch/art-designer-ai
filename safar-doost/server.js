...
const server = http.createServer((req,res)=>{
  ...
  if(raw==="/health"||raw==="/health/"){res.writeHead(200,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"});return res.end(JSON.stringify({ok:true,service:"safar-doost"}));}
  ...
  fs.stat(file,(err,st)=>{
    ...
    fs.readFile(target,(readErr,data)=>{
      ...
      res.writeHead(200,{"content-type":mime[ext]||"application/octet-stream","cache-control":ext===".html"?"no-cache":"public, max-age=3600","x-content-type-options":"nosniff","referrer-policy":"strict-origin-when-cross-origin"});
      res.end(data);
    });
  });
});
